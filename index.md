    ...
    <script>
        ...
        .then(data => {
            ...
            Promise.all(calls).then(function (responses) {
                ...
                var influencers = '<h4>Influencing Organisations</h4>';
                for(s=0; s < sortable.length; s++) {
                    influencers += `<div class="one column">
                    <img src="${sortable[s][2]} alt="${sortable[s][0]}" style="width:32px; height:32px"/> 
                    ${sortable[s][0] + ' ' + sortable[s][1]}</div>
                    `
                    if(s == 10) break;
                }
                document.getElementById("external-influence").innerHTML=influencers
